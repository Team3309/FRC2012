...
public class Gyro extends Subsystem {
	 ...
	 * 
	 * @param twist
	 *            - Joystick twist value - not cubed!
	 * @return
	 */
	public double getTwistRate() {
		...
		if (Math.abs(distance) >= 10) {
			double twistRate = ((MAX_SPEED - MIN_SPEED) / 180)
					* Math.abs(distance) + MIN_SPEED;
	...
	public void updateDesiredHeading(double joystickTwist) {
		desiredHeading += joystickTwist * joystickTwist * joystickTwist
				* (MAX_OFFSET);
